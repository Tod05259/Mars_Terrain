...
VEC = load_by_path("escape_env_vec", "26_escape_env_vec.py")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Draw only the top layer of material. Smaller files, but a "
            "288 mm bed then looks like one sheet of beads. Off by "
            "default so the recording shows the bed that is simulated."
        ),
    )
    parser.add_argument(
        "--soil-spacing",
        ...
        default=0.0125,
        help=(
            "Material point spacing [m], used to size the drawn grains at "
            "half of it. Must match the spacing the snapshot was built "
            "with or the bed will look either gappy or fused."
        ),
    )
    ...
    if cli.draw_surface_only:
        surface_index = torch.tensor(
            np.flatnonzero(env.surface_mask_np), device=env.device, dtype=torch.long
        )
    else:
        # Every material point, so the bed is seen as the solve has it. Drawing
        # only the top layer makes a 288 mm bed read as a single sheet of
        # spheres hanging over the ground, which is not what is being
        # simulated. It costs file size -- roughly seven times -- and that is
        # the right trade when the recording exists to be looked at.
        surface_index = torch.arange(
            env.model.particle_count, device=env.device, dtype=torch.long
        )
    # Material points stand in for a cluster of grains. Drawn at half the
    # spacing they tile into a continuous bed; drawn smaller they read as
    # scattered beads, which is why the radius follows the spacing rather
    # than being fixed.
    grain_radius = 0.5 * cli.soil_spacing
    ...
    for episode in range(cli.max_episodes):
        ...
        for _ in range(env.max_episode_steps):
            ...
            # log_state draws rigid shapes only, so the soil has to be logged
            # separately or the robot appears to walk on nothing. Only the top
            # layer is drawn: it is what is visible anyway, and logging all
            # 76,800 material points per frame bloats the file sevenfold.
        ...
        if escaped:
            ...
            print(
                f"episode {episode}: failed (exit={exit_m:.2f} m), discarded",
                flush=True,
            )

    print(f"\n[record] kept {kept} success recording(s) in {cli.output_dir}")
    if kept:
