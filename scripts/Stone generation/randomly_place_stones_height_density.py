...
--------------------------------------------------------------------------------
NOTE ON HEIGHT-BASED STONE DENSITY
--------------------------------------------------------------------------------
Stones (and their extra small copies) are no longer scattered uniformly. Each
terrain vertex is weighted by its world height so that LOW terrain receives MORE
stones and HIGH terrain receives FEWER. `build_height_sorted_vertex_order()`
performs Efraimidis-Spirakis weighted sampling without replacement to produce a
low-terrain-first ordering, and `pop_next_stone_vertex()` consumes that ordering
for both the main stones and their extra small copies.

Tunables:
  HEIGHT_DENSITY_STRENGTH   0 = uniform (old behavior), higher = stronger low bias
  HEIGHT_DENSITY_MIN_WEIGHT weight floor so the highest peaks still get a few stones

Challenge objects are still placed uniformly at random. The total placed-object
count is unchanged; only the spatial distribution of the stones changes.
"""
...
# --- Height-based stone density ---
# Stones prefer LOW terrain: low areas get more stones, high areas get fewer.
HEIGHT_DENSITY_STRENGTH = 2.0      # 0 = uniform (old behavior), higher = stronger low-terrain bias
HEIGHT_DENSITY_MIN_WEIGHT = 0.02   # weight floor so the highest points still get a few stones
...
def build_height_sorted_vertex_order(terrain, used_vertex_indices,
                                     strength=HEIGHT_DENSITY_STRENGTH,
                                     min_weight=HEIGHT_DENSITY_MIN_WEIGHT):
    """Return terrain vertex indices ordered so LOW vertices are consumed first.

    Each unused vertex i gets a weight that grows as its world height drops:
        h_norm = (z_i - z_min) / (z_max - z_min)   # 0 = lowest, 1 = highest
        w_i    = max((1 - h_norm) ** strength, min_weight)
    Efraimidis-Spirakis weighted sampling without replacement then assigns
        key_i  = random() ** (1 / w_i)
    and we sort by key. Larger weight (lower terrain) yields a larger key on
    average, so those vertices are popped first, while randomness is preserved so
    the result is a smooth density gradient rather than a rigid low-to-high sweep.

    The list is returned in ascending-key order, so the most-preferred (lowest
    terrain) vertex sits at the end and list.pop() (O(1)) returns it next.
    """
    heights = [(terrain.matrix_world @ v.co).z for v in terrain.data.vertices]
    ...
    for idx in range(len(heights)):
        if idx in used_vertex_indices:
            continue

        h_norm = (heights[idx] - z_min) / z_range
        weight = max((1.0 - h_norm) ** strength, min_weight)
...
def pop_next_stone_vertex(terrain, ordered_indices, used_vertex_indices):
    """Pop the next low-terrain-biased unused vertex, or (None, None) if empty."""
    while ordered_indices:
        idx = ordered_indices.pop()
        if idx in used_vertex_indices:
            continue
        used_vertex_indices.add(idx)
        return idx, terrain_vertex_world_location(terrain, idx)
    return None, None
...
# Build a low-terrain-first ordering of the remaining vertices. Stones and their
# extra small copies are consumed from this order, so LOW areas fill up first and
# HIGH areas stay sparser -> fewer stones on high terrain, more on low terrain.
stone_vertex_order = build_height_sorted_vertex_order(terrain, used_vertex_indices)

stone_counter = 0

while len(placed_objects) < TARGET_TOTAL_OBJECTS:
    vertex_index, location = pop_next_stone_vertex(terrain, stone_vertex_order, used_vertex_indices)
    ...
    if half_height <= HEIGHT_HALF_THRESHOLD:
        for extra_id in range(EXTRA_SMALL_STONE_COUNT):
            ...
            extra_vertex_index, extra_location = pop_next_stone_vertex(
                terrain,
                stone_vertex_order,
                used_vertex_indices
            )
