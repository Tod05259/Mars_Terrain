...
"""Train the escape policy with RSL-RL PPO on the vectorised entrapment env.

Batch version of 23_train_escape.py: wraps :class:`EscapeEnvVec`
(26_escape_env_vec.py) — N robots and N soil windows in a single MuJoCo+MPM
solve — in the installed RSL-RL ``VecEnv`` contract and runs
``OnPolicyRunner``. The PPO configuration is imported from 23 so the single-
and multi-env trainers cannot drift apart.

Auto-reset happens inside ``EscapeEnvVec.step``: observations returned for
done envs already belong to the next episode, matching RSL-RL semantics.

Usage
-----
    python terrain/scripts/27_train_escape_vec.py --num-envs 16 --iterations 400 \
        --init-checkpoint <walking-or-escape-checkpoint>
"""
